...
# Function to compare 2 models based on histogram
histogram_p_value <- function(data, model_name_1, model_name_2){
  ...
  plot <- ggplot() + 
    ...
    annotate("text", x= 0.15, y= 10, label = paste0("p-value = ", p_value))
...
perm_p_value <- function(data, model_name_1, model_name_2){
  ...
  auc.null <- do(1000) * diff(mosaic::mean(test_aucs ~ shuffle(model), data = test_subsample))
  p_value <- 2*min(sum(auc.null[,1]>=obs)/1000, sum(auc.null[,1]<=obs)/1000) 
  return(p_value)
}
