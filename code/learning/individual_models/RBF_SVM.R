...
for (i in 1:100) {
  ...
  grid <-  expand.grid(sigma = c(0.05, 0.01, 0.1, 0.9),
                       C = c(0.05, 0.01, 0.05, 0.1))
  ...
  rbf_SVM <- train(dx ~ .,
                   ...
                   metric = "ROC",
                   tuneGrid = grid)
  # Best sigma parameter
  best.tune.sigma <- rbf_SVM$bestTune[1]
  # Best C parameter
  best.tune.cost <- rbf_SVM$bestTune[2]
