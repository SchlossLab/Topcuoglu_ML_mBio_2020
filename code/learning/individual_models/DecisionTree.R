...
for (i in 1:100) {
  ...
  #all.cv.response <- c(all.cv.response, dt$pred$obs)
...
write.table(full, file='data/process/decision_tree_aucs_hps_R.tsv', quote=FALSE, sep='\t', col.names = c("cv_aucs","test_aucs", "Max-depth"), row.names = FALSE)

pdf("results/figures/Decision_Tree_inR.pdf")
par(mar=c(4,4,1,1))
