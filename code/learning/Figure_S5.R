...
logit_files <- list.files(path= 'data/process/', pattern='combined_traintime_L2_Logistic_Regression_.*', full.names = TRUE)
rf_files <- list.files(path= 'data/process/', pattern='combined_traintime_Random_Forest_.*', full.names = TRUE)
...
ggsave("Figure_S5.png", plot = last_plot(), device = 'png', path = 'submission', width = 6, height = 9.5)
