...
logreg = linear_model.LogisticRegression(C=0.001) # coefficient is chosen to increase penalty for weights (overfitting).
```

#### Plot ROC curve for cross validation of training set and then test set 
1. Data is split to 80-20 training-test 100 times in a loop. 
2. Each training set then is split to 80-20; 5 times to do a cross-validation ROC. A mean cross validation ROC is generated.
3. For each epoch, the test set that is generated in the beginning of the loop is predicted by the model and a mean test ROC is generated.
