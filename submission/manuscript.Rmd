...
**Statistical analysis workflow.** Data summaries, statistical analysis, and data visualizations were performed using R (v.3.5.0) with the tidyverse package (v.1.2.1). We compared the performance of the models pairwise by calculating the difference between AUROC values from the same datasplit (for 100 datasplits). We determined if the models were significantly different by calculating the empirical p-value  (2 x min(% of AUROC differences $\geq$ 0, % of AUROC differences $\leq$ 0) for the double tail event (e.g., Figure S3). 
...
**Acknowlegements.**  We thank all the study participants of Great Lakes-New England Early Detection Research Network. We would like to thank the members of the Schloss lab for their valuable feedback. Salary support for M.R came from NIH grant 1R01CA215574. Salary support for P.D.S. came from NIH grants P30DK034933 and 1R01CA215574.
...
**Figure 2. Generalization and classification performance of ML models using AUROC values of all cross validation and testing performances.** The median AUROC for diagnosing individuals with SRN using bacterial abundances was higher than chance (depicted by horizontal line at 0.50) for all the ML models. The predictive performance of random forest model was higher than other ML models, though not significantly (p > 0.01). L2-regularized logistic regression, XGBoost, L2-regularized SVM with linear and radial basis function kernel performances were not significantly different from one another. The boxplot shows quartiles at the box ends and the median as the horizontal line in the box. The whiskers show the farthest points that were not outliers. Outliers were defined as those data points that are not within 1.5 times the interquartile ranges. 
\newpage
...
\makecell[l]{Source \\ of data} & \makecell[l]{Input data are not appropriate \\ for intended application (e.g. input \\ data provided only from \\ patients with carcinomas but \\ model is expected to predict \\ advanced adenomas.)} & \makecell[l]{Input data are generated \\ according to study design and \\ they are appropriate for intended \\ application.} & \makecell[l]{Input data are available \\ to predict intended outcomes \\ and will be available \\ in the future (e.g. same OTU \\ assignments for new fecal \\ samples)} \\ \hline

\makecell[l]{Study \\ cohort} & \makecell[l]{Test data resampled to \\ remove class imbalance. \\ (e.g. test data only includes \\ patients with carcinomas but \\ model is trained to predict \\ healthy colons as well.)}  & \makecell[l]{Test data are reflective \\ of the population which \\ the model will eventually \\ be applied to.} & \makecell[l]{Model tested on multiple \\ cohorts with different \\ class balances.} \\ \hline
...
\makecell[l]{Model \\ development} & \makecell[l]{No hyperparameter tuning.} & \makecell[l]{Model is tested across multiple \\ hyperparameter settings.} & \makecell[l]{Hyperparameter grid search \\ performed by cross-validation \\ on the training set.} \\ \hline
...
\makecell[l]{Evaluation \\ metrics} & \makecell[l]{Reported a single performance \\ metric that is not appropriate for \\ intended application  (e.g. when \\ predicting rare outcome, \\ accuracy metric is not \\ reliable).} & \makecell[l]{Reported a single performance \\ metric that is appropriate \\ for intended application with \\ confidence intervals.} & \makecell[l]{Reported multiple metrics \\ with confidence intervals.} \\ \hline
