...
- decision tree has no rgularization.
...
- Testing with each model will take the same amount of time but if you are training a model and optimize hyper-parameters, then you need to consider the computational burden. The computational efficiency of different models have been investigated before. Explain for each model. In tree based models, depth of tree, number of features, .. matter. L1 linear is very fast.

- But we can reduce time paralellizing different components of the pipeline. Here I have parallelized each datasplit but we can also parallelize cross-validation hyerp-parameter search for each hyper-parameter setting. 
