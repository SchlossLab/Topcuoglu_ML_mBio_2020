...
Members of the human microbiota have been implicated in a wide range of chronic diseases and can be used as biomarkers to diagnose them. However, identifying a set of biomarkers and making generalizable disease diagnoses is challenging because human-associated microbiome is high-dimensional and uneven within and between individuals with the same disease. Machine learning (ML) modeling of human-associated microbial communities is used to overcome this challenge. Progress has been made towards developing ML models that predict health outcomes from bacterial abundances, but reliable and reproducible ML models are scarce due to the flawed modeling methods and a lack of transparency in application and selection of ML models. We established a standard for ML modeling pipelines that use 16 rRNA gene sequences to predict diseases. We used fecal 16S rRNA sequences to predict the presence/absence of colorectal cancer (CRC) lesions in patients (N=490) from a previously published CRC study. We developed seven ML models with different classification algorithms (L2-regularized logistic regression, L1 and L2 support vector machines (SVM) with linear and radial basis function kernels, a decision tree, random forest and extreme gradient boosted decision tree (XGBoost)) to show the effect of model selection on model accuracy, interpretability and computational efficiency. Random forest model was best at detecting CRC lesions but it was slow to train and hard to interpret. Despite the simplicity, the L2-regularized logistic regression followed random forest in predictive performance and it trained the fastest. Our results in comparing seven ML models with one dataset, showed the importance of model selection in ML model development. It also showcased a robust ML pipeline that creates generalizable and reproducible ML models.  
...
#### Interpretation of results
 In this study we established a robust ML pipeline to use 16S rRNA sequence counts to predict a binary health outcome. We showed the importance of held-out test set and reporting the results of many iterations of cross-validation and testing. Each data-split has a weight that comes from random splitting. Seeing that testing and cross-validation median differences being low show that the models are generalizable and the pipeline works. Our results also show that we can choose different models for different reasons. Getting feature importances as weights and regression coefficients is the fastest and easiest interpratation. Similarly, linear models require less computational burden, fastest to train. THinking about the applicability, and actionability of a model in clinical setting, we might want faster results. 
...
#### Consideration of possible weaknesses
 ...
 Does the code work on any microbiome data?
  Why did we use only one dataset? Why not validate on other data instead of held-out testing set? Becuase we are comparing several methods on 1 dataset.
