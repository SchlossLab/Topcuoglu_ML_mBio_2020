...
Table1 <- kable(model_param, "latex", caption = "Optimized hyper-parameters, pre-processing and cross-validation methods and software implementation of the classification algorithms", booktabs = T) %>%
...
param_range <- read.table('../data/process/model_parameters.txt', header=T, sep='\t', na.strings="", stringsAsFactors = FALSE) %>% 
  replace(is.na(.), "-") %>%  
  select(-Value, -Parameter, -Cross.Validation, -Epochs, -Scaler, -Sklearn.Function) 
  
Table2 <- kable(param_range, "latex", caption = "The range of optimized hyper-parameters for each of the classification algorithms", booktabs = T) %>%
kable_styling(latex_options = c("striped", "scale_down"), position = "left")%>%
