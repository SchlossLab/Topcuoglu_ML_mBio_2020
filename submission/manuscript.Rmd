...
# in python we use StandardScaler, here we center and scale to get the same effect
preprocessParams <- preProcess(data[,-1], method=c("center", "scale"))
# transform the dataset using the parameters
data[,-1] <- predict(preprocessParams, data[,-1])
```

```{r BinaryClassification, eval=TRUE, echo=FALSE, cache=FALSE, warning=FALSE, message=FALSE}
# Create lists to store the predicted labels and actuals labels at each iteration 
all.test.response <- all.test.predictor <- test_aucs <- c()

## We will split the dataset 80%-20% and tune hyper-parameter on the 80% training. This tuning step on training data will be done 50 times with 5 folds on the training set and an optimal hyper-parameter/optimal model will be chosen.
...
## We will do the previous steps 50 iterations. Each iteration will have a unique hyper-parameter and optimal model. For each of these models we will have a TEST AUC. We will report the TEST AUC with 95 %CI.

# For each epoch, we can also keep track of AUC values with CI for each cross-validation during training.But these code lines are not included in this Rmd file. I will have those commented out in the main.R code file. 
for (i in 1:50) {
  # Partition the data 80-20. This function in caret keeps it stratified if I remember correctly.
  ...
  # Cost is our regularization parameter which here is the inverse of penalty and we provide a grid to tune the best one. 
  ...
  all.test.response <- c(all.test.response, 
                         ifelse(testing$dx == "cancer", 1, 0))
  # Save the test set predicted probabilities of highest class in all.test.predictor
  all.test.predictor <- c(all.test.predictor, 
                          probs[[2]])
}
# Get the ROC curve and AUC values for all the iterations
# auc= TRUE and ci=TRUE will keep track of the auc and 95 CI for all the sensitivities and specificities. 
test_roc <- roc(all.test.response, 
                all.test.predictor, 
                auc=TRUE, 
                ci=TRUE)
```

```{r LogitROCplot, eval=TRUE, echo=FALSE, cache=FALSE, warning=FALSE, message=FALSE}
# Here we plot the test AUC and 95%CI for logistic regression.
...
# Calculate CI for AUC values
auc_ci <- test_roc$ci[3]-test_roc$ci[2]
...
legend(x=0.7,y=0.2,
       legend=(sprintf('Test AUC: %.3g, %s CI: %0.3g', 
                       test_roc$auc, 
                       '%95', 
                       auc_ci)),
       bty='n',
