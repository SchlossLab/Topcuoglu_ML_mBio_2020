...
{[}Figure 3{]} and permutation importance methods {[}Figure S3{]}.
However, in general L2-regularized models had different OTUs picked out
as important in the two methods of interpretation due to collinearity in
...
Microbiome studies use ML models with a classification task to learn the
training data to assign labels, healthy or not, to new data (e.g.~new
samples), but also to learn which features are important enough to
discriminate between labels. This is a difficult task because we need to
correctly interpret our ML models. In this study we used two methods to
interpret our models. Absolute feature weights of trained models and
permutation importance. When interpreting linear models with the
differences we observed between the two interpretation methods is
possibly due to collinearity in the dataset. Collinearity is that one
OTU is highly correlated with other OTUs which means that the regression
coefficients/weights are not unique and that they are influenced by each
other. If they are influenced and not unique then it becomes harder to
interpret the model. You may want to use different models for prediction
and one for interpretation/inference, in one you remove highly
correlated OTUs to interpret and in other keep them all to predict.
