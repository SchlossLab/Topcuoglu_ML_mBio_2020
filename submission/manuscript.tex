...
highlighting patterns in complex microbial datasets. ML models learn
from existing data to predict the outcomes of new data and allow us to
infer on the reasons underlying that prediction. Therefore, researchers
have started to explore the utility of ML models that use microbiota
...
implemented (12, 13). More notably, we commonly see flawed ML practices
such as using ML pipelines where there is no seperate held-out test
dataset to evaluate model performance or reporting few or only the best
outcomes of cross-validation. Even when there are seperate testing sets
to evaluate model performance, there are large differences between
cross-validation and testing performances that indicate overfitting as
well as large confidence intervals for testing performances (4, 14--20).
...
models require posthoc explanations to determine the feature importances
in making a prediction. These explanations can be misleading and at
times unreliable when making high-stake decisions about someone's health
(24). The lack of transparency on model selection and interpretation as
well as flawed modeling methods negatively impact model validity and
...
cancer (CRC) study (3) which had fecal 16S rRNA gene sequences of 490
...
radial basis kernel, decision tree, random forest and XGBoost, using
permutation importance on the held-out test data where we randomly
permuted non-correlated features individually and groups of correlated
features together (see methods) to calculate the effect of permuted OTUs
or group of OTUs on testing AUROC. The top 5 OTUs with the largest
...
testing AUROC median to 0.52, from 0.69 to 0.68 and from 0.68 to 0.65,
respectively {[}Figure 4{]}.

We used permutation importance to interpret linear models as well to
highlight the differences between the two interpretation methods
{[}Figure S3{]}. The rankings of OTUs that discriminate between having
...
(OTU00822, OTU01239, OTU00609) as important in feature rankings
{[}Figure 3{]} and permutation importance methods {[}Figure S3{]} but
with different ranks. The difference was more pronounced in
L2-regularized models as they had mostly different OTUs picked out as
due to collinearity in microbial communities.
...
values did not exceed 0.021 which suggests that these models are
generalizable and can be used to test similar new data. Second, we
performed the initial 80\%-20\% random datasplit 100 times in our ML
pipeline. The randomization of the initial data-split to create a
...
that there was variability in AUROC values between different random
...
data-split. Third, we used AUROC metric instead of accuracy in our study
to evaluate predictive performance of the ML models. AUROC is always
random at the value 0.5 and is a robust metric when a dataset is
imbalanced. We also performed a full grid search for hyperparameter
settings when building a ML model. It is not enough to use the default
hyperparameter settings when using previously developed ML packages in
programming languages such as R, Python and Matlab. In the example of
...
models with the classification task to learn the training data to assign
labels, such as healthy or not, to new data (e.g.~new samples), but also
to learn which features are important enough to discriminate between
labels. If the goal of a study is not just to predict a health outcome
but also to learn the ecology behind a disease and to identify microbial
biomarkers of a disease, then interpretibility of a ML model is crucial.
In terms of predictive performance, random forest model had testing
...
random forest with a median AUROC difference of only 0.015. In terms of
interpretability, random forest was a more complex ML model and it could
...
Even with interpretable models, we need to be careful with how we treat
the information we get from the models. In this study we used two
methods to interpret our linear models; absolute feature weights of
trained models and permutation importance. We observed differences in
outcome between the two interpretation methods due to collinearity in
the dataset. Collinearity in a microbial dataset is when one OTU is
highly correlated with another OTU which causes regression coefficients
to not be unique. The regression coefficients of correlated OTUs are
influenced by one another which makes it difficult to interpret a ML
model. There are ways to make it easier to interpret models.; (1) use
different models for prediction and interpretation in which highly
correlated OTUs are removed, (2) get the highly ranked OTUs and check
what othe OTUs they are correlated with. The interpretation however
should not be a final answer but should be used to generate hypothesis
that are tested with followup experiments.

There are other criteria when choosing ML models such as the
computational burden of developing it, sample size and balance of the
dataset. In terms of computation burden, random forest model was slow to
train 83.2 hours whereas L2-regularized logistic regression trained in
12 minutes. The generalization performance of ML models depends on
sample size. The more complex the model, the more data it will need. The
