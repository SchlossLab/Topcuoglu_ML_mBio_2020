...
models are scarce due to the flawed modeling methods that call into
question the validity of developed ML models. Furthermore, the use of
...
with an AUROC of 0.695 but it was slow to train (101.3 h) and hard to
...
there are seperate testing sets to evaluate model performance, we
observe large differences between cross-validation and testing
performances that indicate overfitting as well as large confidence
intervals for testing performances (4, 13--19). Moreover, there is a
lack of discussion on why a particular ML model is utilized. Recently,
there is a trend towards using more complex ML models such as random
forest, extreme gradient boosting and neural networks without a
discussion on if and how much model interpretibility is necessary for
the study (11, 20--22). Black box machine learning models require
posthoc explanations to determine the importances of features in making
a prediction. These explanations can be misleading and at times
unreliable when making high-stake decisions about someone's health (23).
The lack of transparency on modeling methodology, model interpretation
and model selection negatively impact model reproducibility and
validity. We need to strive toward better machine learning practices by
(1) implementing consistent and rigourous machine learning pipelines and
...
microbiome is hypothesized to directly contribute to its development
therefore fecal 16S rRNA gene sequences can be used to detect CRC. We
built ML models using fecal 16S rRNA gene sequences to predict healthy
...
We established a robust ML pipeline where we train and validate each of
the seven models {[}Figure 1{]}. We randomly split the data into
...
hyper-parameter setting in terms of AUROC to train the full
...
dataset which showed that depending on the data-split, the testing AUROC
value showed great variability {[}Figure 2{]}. The testing AUROC values
within each model varied 0.23 on average across the seven models. For
instance, the lowest AUROC value of the random forest model was 0.59
whereas the highest was 0.81.
...
predictive performances the most for all tree-based methods {[}Figure
...
Decision tree was more susceptible to The negative predictive impact of
...
As the complexity of a ML model increased {[}Table S1{]}, its training
times increased as well {[}Figure 5{]}. Additionally, the more
hyperparameters a model had {[}Table S1{]}, the longer it took to train
{[}Figure 5{]}. Linear models trained faster than non-linear models. L2
logistic Rregression and L1 and L2 SVM with linear kernel had training
times of 0.2 hours, (std ± 0.03), 0.2 hours, (std ± 0.02), and 0.2
hours, (std ± 0.02), respectively. Whereas, SVM with radial basis
function kernel, decision tree, random forest and xgboost had training
...
testing AUROC values. When the differences between cross-validation and
test performance is low, this suggets the models will perform similar
with similar data. In all seven models, the differences in
...
In our ML pipeline we performed the 80\%-20\% random datasplit 100
times. The repetition of the initial data-split to create a held-out
test set is a crucial step in the ML pipeline to develop generalizable
ML models and to report reliable performance metrics for a ML model.
Depending on the data-split, there is the chance of being overoptimistic
about the predictive performance of a model. In our study, we showed
that there are differences in AUROC values between different random
data-splits in each of the models we tested. Our results showed that the
testing AUROC values varied 0.23 on average, depending on the
data-split.

Our results also showed that we should choose to use ML models based on
...
performance, all models except a decision tree and L1-regularized SVM
with linear kernel behaved similarly. Random forest model had testing
AUROC values statistically significantly larger than others. However the
second best model was L2-regularized logistic regression following
random forest with a median AUROC difference of only 0.015. The decision
tree model performed poorly because it had no regularization.
L1-regularized SVM with linear kernel also performed poorly possibly
because it was more sensitive to hyper-parameter tuning. In terms of
interpretation, random forest is a more complex ML model and it can only
be explained using methods such as permutation importance. On the other
hand, L2-regularized logistic regression is easy to interpret
(i.e.~regression coefficients of the trained model). If the goal of the
study is to learn the ecology behind a disease and to identify
biomarkers, then easily interpretable models could be a better fit. It
is also important to consider the computational burden of developing ML
models. Random forest model was slow to train with upto \texttt{r} hours
whereas L2-regularized logistic regression trained in \texttt{r}
minutes.

Our study underscored the importance of performing a full grid search
for hyperparameter settings when building a ML model. We first performed
a granular grid search and then a full grid search on the limited range
of possible hyperparameter settings. In the example of L1-regularized
SVM with linear kernel, the model showed variability between different
regularization coefficients (C).
...
\begin{itemize}
\item
  What happens with imbalanced data The metric you use as the
  discriminative performance becomes important. Don't use accuracy.
  AUROC is good and random at 0.5 always.
\item
  What happens with smaller datasets Generalization performance depends
  on size. The more complex the model, more data you will need. Use
  linear models if you have little data.
\item
  Does the code work on any microbiome data? So if yu have a new sample,
  do you have the same OTUs? NO
\item
...
efficiency of seven ML models.} The traintimes for training and testing
of each data-split showed the differences in computational efficieny of
the seven models. The median traintime in hours was the highest for
