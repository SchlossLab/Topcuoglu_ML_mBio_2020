...
highlights the importance of choosing an ML approach should be chosen
based on the goal of the study, as the choice will inform expectations
...
random forest model had a median AUROC value of 0.695 {[}IQR
0.650-0.739{]} which was higher than the other models for detecting
SRNs. However, we determined that the random forest model performance
was not statistically significantly higher (p = 0.5) when we
investigated the distribution of differences between random forest and
L2-regularized logistic regression AUROC values for each data-split
...
Although permutation analysis allowed us to gauge the importance of an
OTU, the analysis was post hoc (i.e., done using the test data).
...
predictions is a significantly more challenging and distinct
undertaking. For example, we currently lack standardized methods to
collect patient samples, generate sequence data, and report clinical
data. We are also challenged by the practical constraints of OTU-based
...
are added (41). Finally, we also need independent validation cohorts to
...
(42). Patients with an adenoma greater than 1 cm, more than three
...
generated by Sze et al (43), who processed the 16S rRNA sequences in
...
to OTUs at 97\% similarity using the OptiClust algorithm (41, 44, 45);
...
Recently developed tools such as Hyperband help researchers with
hyperparameter selection that can be incorporated to microbiome studies
(46). For L2-regularized logistic regression, L1 and L2-regularized SVM
with linear and radial basis function kernels, we tuned the cost
...
models by taking the difference of AUROC values from the same datasplit.
We plotted a histogram that showed the AUROC differences for 100
datasplits and calculated the empirical p-value of the histogram (2 x
min(\% of AUROC differences \(\geq\) 0, \% of AUROC differences \(\leq\)
0) for double tail event (e.g., Figure S3).
...
absolute feature weights of (A) L2-regularized logistic regression, (B)
...
43. \textbf{Sze MA}, \textbf{Schloss PD}. 2018. Leveraging existing 16S
...
46. \textbf{Li L}, \textbf{Jamieson K}, \textbf{DeSalvo G},
